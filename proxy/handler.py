...
class ProxyHandler:
    ...
    def _handle_socks5(self) -> None:
        ...
        if atyp == SOCKS5_ATYP_IPV4:
            ...
            host = self._recv_exact(length).decode()
    ...
    def _rewrite_request(self, raw: bytes, req: HTTPRequest) -> bytes:
        if not (req.target.startswith("http://") or req.target.startswith("https://")):
            return raw
        old_line = f"{req.method} {req.target} {req.version}".encode()
        new_line = f"{req.method} {req.path} {req.version}".encode()
        return raw.replace(old_line, new_line, 1)
